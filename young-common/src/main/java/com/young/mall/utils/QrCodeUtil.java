...
public class QrCodeUtil {
     ...
     * @return
     */
    public String createGrouponShareImage(String goodName, String goodPicUrl, YoungGroupon groupon, BigDecimal counterPrice, BigDecimal retailPrice) {
        try {
            // 创建该商品的二维码
     ...
     * @return
     */
    private byte[] drawPicture(InputStream qrCodeImg, String goodPicUrl,
                               String goodName, BigDecimal counterPrice,
                               BigDecimal retailPrice) throws IOException {
        //底图
        ClassPathResource readResource = new ClassPathResource("back.png");
        BufferedImage read = ImageIO.read(readResource.getInputStream());
        ...
        // --- 开始画图 ---
        // 底层空白 bufferedImage
        BufferedImage baseImage = new BufferedImage(read.getWidth(), read.getHeight(), BufferedImage.TYPE_4BYTE_ABGR_PRE);

        //画上图片
        drawImgInImg(baseImage, read, 0, 0, read.getWidth(), read.getHeight());
        // 画上商品图片
    ...
    private void drawImgInImg(BufferedImage baseImage, BufferedImage imageToWrite,
        ...
        Graphics2D graphics = (Graphics2D) baseImage.getGraphics();
        graphics.drawImage(imageToWrite, x, y, width, heigth, null);
        graphics.dispose();
    }
