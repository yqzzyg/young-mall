...
public class OrderUtil {
    ...
    public static String orderStatusText(YoungOrder order) {
    ...
    public static OrderHandleOption build(YoungOrder order) {
        int status = order.getOrderStatus().intValue();
    ...
    public static String orderHtmlText(YoungOrder order, String orderIds, List<YoungOrderGoods> orderGoodsList) {
        String orderStr = "";
        if (orderGoodsList == null || orderGoodsList.size() < 1) {// 无商品时，设置商品信息为null
            orderStr = null;
        } else {
            for (YoungOrderGoods dog : orderGoodsList) {
                orderStr = orderStr
